...
Danish_Monarchs <- read_csv("data/Danish_Monarchs - Sheet1.csv")
```
...
```{r }
ggplot(data = Western_Europe, aes(Year, homicides_per_100k, colour = Entity)) +
  geom_line() +
  labs(x = "Year",
...
```{r}
ggplot(data = Danish_Monarchs, aes(ReignToYear - ceiling((ReignToYear-ReignFromYear+1)/2), 
                                   ReignToYear-ReignFromYear+1)) +
  geom_point() +
  geom_smooth() +
  labs(x = "Mid year",
       y = "Reign duration in years",
       title = "Reign duration smooth")
```
